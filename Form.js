class Form{
    constructor(){
       this.input = createInput("Name");
       this.button = createButton('Play');
       this.greeting = createElement('h2');
       this.title = createElement('h2');
       this.reset = createButton('Reset');
    }
    hide() {
        this.greeting.hide();
        this.button.hide();
        this.input.hide();
        this.title.hide();
    }
    display() {
        this.title.html("Treasure Island");
        this.title.position(310, 50);
        this.title.style('font-size', '70px');
        this.title.style('color', 'darkgreen');
        this.input.position(425,400);
        this.input.style('width', '200px');
        this.input.style('height', '20px');
        this.input.style('background', 'white');
        this.button.position(430,500);
        this.button.style('width', '200px');
        this.button.style('height', '40px');
        this.button.style('background', 'green');
        this.reset.position(900, 660);
        this.reset.style('width', '100px');
        this.reset.style('height', '30px');
        this.reset.style('background', 'lightpink');

       console.log(gameState);
        this.button.mousePressed(() => {
            this.input.hide();
            this.button.hide();
           // player.name = this.input.value();
            var player1 = this.input.value();
            this.greeting.html("Hello " + player1);
            gameState = 1;
            this.greeting.position(400,250);
            this.greeting.style('color', 'green');
            this.greeting.style('font-size', '100px');
        });

        // Make reset button reset the values in the database
        
        
        
    }
}